# 発注管理機能統合 実装プラン

最終更新: 2025-11-02

## 📋 概要

本プランは、既存の売上・在庫管理システムに発注管理機能を統合するための段階的実装計画です。
10フェーズに分割し、各フェーズで動作確認を行いながら確実に進めます。

**前提条件**:
✅ スプレッドシート構造の整備完了
- 商品マスタに「ブランド」列を追加済み
- 仕入れマスタシート作成済み
- 発注管理シート作成済み

---

## 🎯 全体目標

1. **GAS側**: 書込機能を追加し、発注CRUD + メール送信APIを実装
2. **Next.js側**: RESTful なプロキシルートを追加
3. **UI側**: `/orders` タブを追加し、一覧・作成・編集機能を提供
4. **セキュリティ**: 既存の認証方式（Google OAuth + allowlist）を維持
5. **一貫性**: 既存UIとの整合性を保ち、shadcn/ui コンポーネントを再利用

---

## 📐 実装フェーズ一覧

```
Phase 1  ━━━ GAS基盤整備（書込権限・doPost・ユーティリティ拡張）
Phase 2  ━━━ GAS発注CRUD API（作成・取得・更新・ステータス遷移）
Phase 3  ━━━ GASメール送信API（依頼メール・納品完了メール）
Phase 4  ━━━ GASマスタ検索API拡張（仕入れマスタ結合・商品検索）
         ↓
Phase 5  ━━━ Next.jsサーバルート実装（発注API群のプロキシ）
Phase 6  ━━━ 型定義とスキーマ整備（TypeScript types・バリデーション）
         ↓
Phase 7  ━━━ UI実装 - 発注一覧ページ（/orders）
Phase 8  ━━━ UI実装 - 新規発注作成ページ（/orders/create）
Phase 9  ━━━ UI実装 - マスタ閲覧ページ（/masters）
         ↓
Phase 10 ━━━ 統合・テスト・デプロイ準備
```

---

## Phase 1: GAS基盤整備 🔴 最優先

**目的**: GASで書込処理を可能にし、POST/PUT/DELETEに対応する基盤を構築

### 作業内容

#### 1.1 スコープ変更
**ファイル**: `apps/gas/src/appsscript.json`

```json
{
  "timeZone": "Asia/Tokyo",
  "exceptionLogging": "STACKDRIVER",
  "oauthScopes": [
    "https://www.googleapis.com/auth/spreadsheets",  // readonly → 読み書き可能に変更
    "https://www.googleapis.com/auth/script.send_mail"  // メール送信用に追加
  ],
  "runtimeVersion": "V8",
  "webapp": {
    "access": "ANYONE",
    "executeAs": "USER_DEPLOYING"
  }
}
```

#### 1.2 doPost実装
**ファイル**: `apps/gas/src/Code.js`

```javascript
// 既存のdoGet()に加えて追加
function doPost(e) {
  try {
    guard_(e); // 認証チェック（既存）
    
    const payload = JSON.parse(e.postData.contents || '{}');
    const action = e.parameter.action;
    
    // アクションのルーティング
    if (action === 'createOrder') return handleCreateOrder_(payload);
    if (action === 'updateOrder') return handleUpdateOrder_(payload);
    if (action === 'changeStatus') return handleChangeStatus_(payload);
    if (action === 'sendRequestEmail') return handleSendRequestEmail_(payload);
    if (action === 'sendDeliveryEmail') return handleSendDeliveryEmail_(payload);
    
    return json_({ error: 'unknown action' }, 400);
  } catch (err) {
    Logger.log('doPost error: ' + err.toString());
    return json_({ error: String(err), stack: err.stack }, 500);
  }
}

// PUT/DELETEシミュレーション用
function doPut(e) { return doPost(e); }
function doDelete(e) { return doPost(e); }
```

#### 1.3 ユーティリティ拡張
**ファイル**: `apps/gas/src/util.js`

```javascript
// 既存のutil.js に以下を追加

/**
 * 日付文字列をDateオブジェクトに変換
 * @param {string} str YYYY-MM-DD形式
 * @return {Date|null}
 */
function parseDate_(str) {
  if (!str) return null;
  var match = String(str).match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!match) return null;
  return new Date(parseInt(match[1]), parseInt(match[2]) - 1, parseInt(match[3]));
}

/**
 * DateオブジェクトをYYYY-MM-DD形式に変換
 * @param {Date} date
 * @return {string}
 */
function formatDate_(date) {
  if (!date || !(date instanceof Date)) return '';
  return Utilities.formatDate(date, 'Asia/Tokyo', 'yyyy-MM-dd');
}

/**
 * DateオブジェクトをYYYY-MM-DD HH:MM:SS形式に変換
 * @param {Date} date
 * @return {string}
 */
function formatDateTime_(date) {
  if (!date || !(date instanceof Date)) return '';
  return Utilities.formatDate(date, 'Asia/Tokyo', 'yyyy-MM-dd HH:mm:ss');
}

/**
 * 四捨五入
 * @param {number} value
 * @param {number} decimals 小数点以下桁数（デフォルト0）
 * @return {number}
 */
function round_(value, decimals) {
  decimals = decimals || 0;
  var factor = Math.pow(10, decimals);
  return Math.round(value * factor) / factor;
}
```

#### 1.4 設定ファイル作成
**ファイル**: `apps/gas/src/config.js`（新規）

```javascript
/**
 * 発注管理用設定
 */
var CONFIG = {
  // スプレッドシートID（環境に応じて変更）
  SPREADSHEET_ID: PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID') || 'YOUR_SHEET_ID',
  
  // シート名
  SHEETS: {
    ORDERS: '発注管理',
    PRODUCT_MASTER: '商品マスタ',
    PURCHASE_MASTER: '仕入れマスタ',
    PRODUCT_STATE: '商品状態',
    DAILY_SALES: '日次売上集計',
    SKU_SALES_DAILY: '商品別日次売上集計',
    BUYBOX_DAILY: '商品別カート取得率集計',
    STOCK_SKU_DAILY: '商品別在庫日次集計',
    STOCK_GLOBAL_DAILY: '全体在庫日次集計'
  },
  
  // メール設定
  MAIL: {
    SUPROLE: PropertiesService.getScriptProperties().getProperty('MAIL_SUPROLE') || 'suprole@example.com',
    BEFREE: PropertiesService.getScriptProperties().getProperty('MAIL_BEFREE') || 'befree@example.com'
  },
  
  // ページネーション
  PAGINATION: {
    ITEMS_PER_PAGE: 50
  },
  
  // ステータス遷移定義
  STATUS_TRANSITIONS: {
    '依頼中': ['営業倉庫', 'FBA', '返品'],
    '営業倉庫': ['FBA', '返品'],
    'FBA': [],
    '返品': []
  }
};
```

### テスト内容
1. GASエディタで再デプロイ（権限承認が必要）
2. doPost()が呼び出せることを確認
3. Script Propertiesに以下を設定:
   - `SPREADSHEET_ID`: 実際のシートID
   - `MAIL_SUPROLE`: 送信先メールアドレス
   - `MAIL_BEFREE`: 送信先メールアドレス

### 完了条件
✅ スコープ変更が反映され、シートへの書込権限が付与されている  
✅ doPost()経由でactionルーティングが動作する  
✅ 新しいユーティリティ関数がエラーなく実行できる

---

## Phase 2: GAS発注CRUD API実装 🔴 最優先

**目的**: 発注の作成・取得・更新・ステータス変更のAPIを実装

### 作業内容

#### 2.1 シートユーティリティ拡張
**ファイル**: `apps/gas/src/sheets.js`

```javascript
// 既存のsheets.jsに以下を追加

/**
 * 発注管理シートを開く
 */
function openOrdersSheet_() {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CONFIG.SHEETS.ORDERS);
  if (!sheet) throw new Error('発注管理シートが見つかりません: ' + CONFIG.SHEETS.ORDERS);
  return sheet;
}

/**
 * 商品マスタシートを開く
 */
function openProductMasterSheet_() {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CONFIG.SHEETS.PRODUCT_MASTER);
  if (!sheet) throw new Error('商品マスタシートが見つかりません');
  return sheet;
}

/**
 * 仕入れマスタシートを開く
 */
function openPurchaseMasterSheet_() {
  var ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
  var sheet = ss.getSheetByName(CONFIG.SHEETS.PURCHASE_MASTER);
  if (!sheet) throw new Error('仕入れマスタシートが見つかりません');
  return sheet;
}

/**
 * シートの全データを列名付きオブジェクト配列で取得
 * @param {Sheet} sheet
 * @return {Array<Object>}
 */
function readSheetAsObjects_(sheet) {
  var values = sheet.getDataRange().getValues();
  if (values.length === 0) return [];
  
  var headers = values[0];
  var data = [];
  
  for (var i = 1; i < values.length; i++) {
    var row = {};
    for (var j = 0; j < headers.length; j++) {
      row[String(headers[j])] = values[i][j];
    }
    data.push(row);
  }
  
  return data;
}
```

#### 2.2 発注ID生成
**ファイル**: `apps/gas/src/handlers/orders.js`（新規）

```javascript
/**
 * 発注ID生成（PO-YYYYMMDD-NNNN形式）
 * @return {string}
 */
function generatePoId_() {
  var today = new Date();
  var dateStr = Utilities.formatDate(today, 'Asia/Tokyo', 'yyyyMMdd');
  var prefix = 'PO-' + dateStr + '-';
  
  var sheet = openOrdersSheet_();
  var data = readSheetAsObjects_(sheet);
  
  // 今日の発注IDを抽出
  var todayOrders = data.filter(function(row) {
    return String(row.po_id || '').indexOf(prefix) === 0;
  });
  
  // 最大連番を取得
  var maxSeq = 0;
  todayOrders.forEach(function(row) {
    var match = String(row.po_id).match(/-(\d{4})$/);
    if (match) {
      var seq = parseInt(match[1]);
      if (seq > maxSeq) maxSeq = seq;
    }
  });
  
  // 次の連番
  var nextSeq = maxSeq + 1;
  var seqStr = ('0000' + nextSeq).slice(-4);
  
  return prefix + seqStr;
}
```

#### 2.3 発注取得API
**ファイル**: `apps/gas/src/handlers/orders.js`

```javascript
/**
 * 発注一覧取得（フィルタ・ソート対応）
 * @param {Object} filters
 * @return {Array<Object>}
 */
function getOrders_(filters) {
  filters = filters || {};
  
  var sheet = openOrdersSheet_();
  var orders = readSheetAsObjects_(sheet);
  
  // 商品マスタと結合（商品名を付与）
  var productMap = getProductMasterMap_();
  
  orders = orders.map(function(order) {
    var sku = String(order.SKU || '');
    var product = productMap[sku] || {};
    
    return {
      po_id: String(order.po_id || ''),
      sku: sku,
      asin: String(order.ASIN || ''),
      productCode: String(order['商品コード'] || ''),
      productName: product.name || String(order['商品名'] || ''),
      orderDate: formatDate_(order['発注日']),
      seller: String(order['発注先セラー'] || ''),
      quantity: num_(order['発注数量（個）']),
      setCount: num_(order['セット数']),
      setSize: num_(order['セット個数']),
      unitPrice: num_(order['単価（税抜/個）']),
      subtotal: num_(order['税抜純売上高']),
      taxRate: num_(order['消費税率']),
      invoiceNo: String(order['伝票No.'] || ''),
      arrivalDate: formatDate_(order['到着予定日']),
      status: String(order['ステータス'] || ''),
      remarks: String(order['備考'] || ''),
      createdBy: String(order.created_by || ''),
      createdAt: formatDateTime_(order.created_at),
      lastUpdatedBy: String(order.last_updated_by || ''),
      lastUpdatedAt: formatDateTime_(order.last_updated_at)
    };
  });
  
  // フィルタ適用
  if (filters.po_id) {
    orders = orders.filter(function(o) { return o.po_id === filters.po_id; });
  }
  if (filters.statuses && filters.statuses.length > 0) {
    orders = orders.filter(function(o) { return filters.statuses.indexOf(o.status) >= 0; });
  }
  if (filters.sku) {
    orders = orders.filter(function(o) { return o.sku === filters.sku; });
  }
  if (filters.asin) {
    orders = orders.filter(function(o) { return o.asin === filters.asin; });
  }
  if (filters.seller) {
    orders = orders.filter(function(o) { return o.seller === filters.seller; });
  }
  if (filters.productName) {
    var keyword = String(filters.productName).toLowerCase();
    orders = orders.filter(function(o) { 
      return o.productName.toLowerCase().indexOf(keyword) >= 0; 
    });
  }
  if (filters.fromDate) {
    orders = orders.filter(function(o) { return o.orderDate >= filters.fromDate; });
  }
  if (filters.toDate) {
    orders = orders.filter(function(o) { return o.orderDate <= filters.toDate; });
  }
  
  // ソート（発注日降順がデフォルト）
  orders.sort(function(a, b) {
    if (a.orderDate > b.orderDate) return -1;
    if (a.orderDate < b.orderDate) return 1;
    return 0;
  });
  
  return orders;
}

/**
 * 商品マスタをマップで取得
 */
function getProductMasterMap_() {
  var sheet = openProductMasterSheet_();
  var products = readSheetAsObjects_(sheet);
  var map = {};
  
  products.forEach(function(p) {
    var sku = String(p.SKU || '');
    map[sku] = {
      asin: String(p.ASIN || ''),
      productCode: String(p['商品コード'] || ''),
      name: String(p['商品名'] || ''),
      brand: String(p['ブランド'] || '')
    };
  });
  
  return map;
}

/**
 * 発注詳細取得
 */
function getOrderById_(poId) {
  var orders = getOrders_({ po_id: poId });
  return orders.length > 0 ? orders[0] : null;
}

/**
 * GETリクエストハンドラ（発注一覧）
 */
function handleGetOrders_(e) {
  var p = e.parameter || {};
  var filters = {
    po_id: p.po_id,
    statuses: p.statuses ? String(p.statuses).split(',') : null,
    sku: p.sku,
    asin: p.asin,
    seller: p.seller,
    productName: p.productName,
    fromDate: p.fromDate,
    toDate: p.toDate
  };
  
  var orders = getOrders_(filters);
  
  return json_({ items: orders, total: orders.length });
}
```

#### 2.4 発注作成API
**ファイル**: `apps/gas/src/handlers/orders.js`

```javascript
/**
 * 発注作成
 * @param {Object} input
 * @return {Object}
 */
function createOrder_(input) {
  // バリデーション
  if (!input.sku) throw new Error('SKUは必須です');
  if (!input.setCount || input.setCount <= 0) throw new Error('セット数は1以上を指定してください');
  
  // 商品マスタから情報取得
  var productMap = getProductMasterMap_();
  var product = productMap[input.sku];
  if (!product) throw new Error('商品マスタに存在しないSKUです: ' + input.sku);
  
  // 仕入れマスタから情報取得
  var purchaseInfo = getPurchaseInfo_(product.asin);
  if (!purchaseInfo) throw new Error('仕入れマスタに存在しないASINです: ' + product.asin);
  
  // 最小ロットチェック
  if (purchaseInfo.minLot > 0 && input.setCount % purchaseInfo.minLot !== 0) {
    throw new Error('セット数は最小ロット(' + purchaseInfo.minLot + ')の倍数で指定してください');
  }
  
  // 計算
  var setSize = purchaseInfo.setSize;
  var unitPrice = round_(purchaseInfo.purchasePrice / setSize, 2);
  var quantity = input.setCount * setSize;
  var subtotal = unitPrice * quantity;
  var taxRate = input.taxRate !== undefined ? input.taxRate : 0.1;
  
  // po_id生成
  var poId = generatePoId_();
  
  // 発注レコード作成
  var now = new Date();
  var orderDate = input.orderDate ? parseDate_(input.orderDate) : now;
  
  var orderRow = [
    poId,                           // po_id
    input.sku,                      // SKU
    product.asin,                   // ASIN
    product.productCode,            // 商品コード
    product.name,                   // 商品名
    orderDate,                      // 発注日
    input.seller || 'Suprole',     // 発注先セラー
    quantity,                       // 発注数量（個）
    input.setCount,                 // セット数
    setSize,                        // セット個数
    unitPrice,                      // 単価（税抜/個）
    subtotal,                       // 税抜純売上高
    taxRate,                        // 消費税率
    '',                             // 伝票No.
    null,                           // 到着予定日
    '依頼中',                       // ステータス
    input.remarks || '',            // 備考
    input.createdBy || '',          // created_by
    now,                            // created_at
    input.createdBy || '',          // last_updated_by
    now                             // last_updated_at
  ];
  
  // シートに追加
  var sheet = openOrdersSheet_();
  sheet.appendRow(orderRow);
  
  Logger.log('発注作成成功: ' + poId);
  
  return { success: true, po_id: poId };
}

/**
 * 仕入れマスタ情報取得
 */
function getPurchaseInfo_(asin) {
  var sheet = openPurchaseMasterSheet_();
  var data = readSheetAsObjects_(sheet);
  
  for (var i = 0; i < data.length; i++) {
    if (String(data[i].ASIN) === asin) {
      return {
        setSize: num_(data[i]['セット個数']) || 1,
        minLot: num_(data[i]['最小ロット']) || 1,
        purchasePrice: num_(data[i]['仕入れ値（税抜/セット）']),
        hazard: data[i]['危険物'] === true || data[i]['危険物'] === 'TRUE',
        hasExpiry: data[i]['消費期限要'] === true || data[i]['消費期限要'] === 'TRUE'
      };
    }
  }
  
  return null;
}

/**
 * POSTハンドラ（発注作成）
 */
function handleCreateOrder_(payload) {
  return json_(createOrder_(payload));
}
```

#### 2.5 発注更新API
**ファイル**: `apps/gas/src/handlers/orders.js`

```javascript
/**
 * 発注更新
 * @param {Object} payload
 */
function updateOrder_(payload) {
  if (!payload.po_id) throw new Error('po_idは必須です');
  
  var sheet = openOrdersSheet_();
  var data = sheet.getDataRange().getValues();
  var headers = data[0];
  
  // po_idの列インデックス取得
  var poIdColIndex = headers.indexOf('po_id');
  if (poIdColIndex === -1) throw new Error('po_id列が見つかりません');
  
  // 対象行を検索
  var targetRowIndex = -1;
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][poIdColIndex]) === payload.po_id) {
      targetRowIndex = i + 1; // シート上の行番号（1-indexed）
      break;
    }
  }
  
  if (targetRowIndex === -1) {
    throw new Error('発注が見つかりません: ' + payload.po_id);
  }
  
  // 更新可能な項目
  var updates = {
    '消費税率': payload.taxRate !== undefined ? payload.taxRate : null,
    '伝票No.': payload.invoiceNo !== undefined ? payload.invoiceNo : null,
    '到着予定日': payload.arrivalDate ? parseDate_(payload.arrivalDate) : null,
    '備考': payload.remarks !== undefined ? payload.remarks : null
  };
  
  // 監査項目
  updates['last_updated_by'] = payload.updatedBy || '';
  updates['last_updated_at'] = new Date();
  
  // 列ごとに更新
  for (var key in updates) {
    if (updates[key] === null) continue;
    
    var colIndex = headers.indexOf(key);
    if (colIndex !== -1) {
      sheet.getRange(targetRowIndex, colIndex + 1).setValue(updates[key]);
    }
  }
  
  Logger.log('発注更新成功: ' + payload.po_id);
  
  return { success: true };
}

/**
 * POSTハンドラ（発注更新）
 */
function handleUpdateOrder_(payload) {
  return json_(updateOrder_(payload));
}
```

#### 2.6 ステータス変更API
**ファイル**: `apps/gas/src/handlers/orders.js`

```javascript
/**
 * ステータス変更
 * @param {Object} payload
 */
function changeOrderStatus_(payload) {
  if (!payload.po_id) throw new Error('po_idは必須です');
  if (!payload.newStatus) throw new Error('newStatusは必須です');
  
  // 現在のステータス取得
  var order = getOrderById_(payload.po_id);
  if (!order) throw new Error('発注が見つかりません: ' + payload.po_id);
  
  var currentStatus = order.status;
  var newStatus = payload.newStatus;
  
  // 遷移可能かチェック
  var allowedTransitions = CONFIG.STATUS_TRANSITIONS[currentStatus] || [];
  if (allowedTransitions.indexOf(newStatus) === -1) {
    throw new Error('ステータス遷移不可: ' + currentStatus + ' → ' + newStatus);
  }
  
  // ステータス更新
  var sheet = openOrdersSheet_();
  var data = sheet.getDataRange().getValues();
  var headers = data[0];
  
  var poIdColIndex = headers.indexOf('po_id');
  var statusColIndex = headers.indexOf('ステータス');
  var updatedByColIndex = headers.indexOf('last_updated_by');
  var updatedAtColIndex = headers.indexOf('last_updated_at');
  
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][poIdColIndex]) === payload.po_id) {
      var rowIndex = i + 1;
      sheet.getRange(rowIndex, statusColIndex + 1).setValue(newStatus);
      sheet.getRange(rowIndex, updatedByColIndex + 1).setValue(payload.updatedBy || '');
      sheet.getRange(rowIndex, updatedAtColIndex + 1).setValue(new Date());
      break;
    }
  }
  
  Logger.log('ステータス変更成功: ' + payload.po_id + ' (' + currentStatus + ' → ' + newStatus + ')');
  
  return { success: true };
}

/**
 * POSTハンドラ（ステータス変更）
 */
function handleChangeStatus_(payload) {
  return json_(changeOrderStatus_(payload));
}
```

### テスト内容
1. cURLまたはPostmanで発注作成APIをテスト
2. 発注一覧取得でデータが返却されることを確認
3. 発注更新・ステータス変更が正常に動作することを確認

### 完了条件
✅ 発注作成APIが動作し、シートに正しくデータが追加される  
✅ 発注一覧取得でフィルタが正常に機能する  
✅ ステータス遷移の検証が正しく行われる

---

## Phase 3: GASメール送信API実装 🟡 重要

**目的**: 依頼メール・納品完了メールの送信機能を実装

### 作業内容

#### 3.1 メールハンドラ実装
**ファイル**: `apps/gas/src/handlers/mail.js`（新規）

```javascript
/**
 * 依頼メール送信（befree宛）
 * @param {Object} payload { poIds: string[] }
 */
function sendRequestEmail_(payload) {
  if (!payload.poIds || payload.poIds.length === 0) {
    throw new Error('送信対象の発注IDが指定されていません');
  }
  
  // 対象発注を取得（依頼中のみ）
  var orders = getOrders_({ statuses: ['依頼中'] });
  var targetOrders = orders.filter(function(o) {
    return payload.poIds.indexOf(o.po_id) >= 0;
  });
  
  if (targetOrders.length === 0) {
    throw new Error('送信対象の発注がありません（ステータス: 依頼中）');
  }
  
  // メール本文生成
  var subject = '【発注依頼】Suprole - ' + today_() + ' (' + targetOrders.length + '件)';
  var htmlBody = buildRequestEmailHtml_(targetOrders);
  
  // 送信
  MailApp.sendEmail({
    to: CONFIG.MAIL.BEFREE,
    subject: subject,
    htmlBody: htmlBody
  });
  
  Logger.log('依頼メール送信: ' + targetOrders.length + '件');
  
  return { success: true, sentCount: targetOrders.length };
}

/**
 * 納品完了メール送信（Suprole宛）
 * @param {Object} payload { poIds: string[] }
 */
function sendDeliveryEmail_(payload) {
  if (!payload.poIds || payload.poIds.length === 0) {
    throw new Error('送信対象の発注IDが指定されていません');
  }
  
  // 対象発注を取得（営業倉庫のみ）
  var orders = getOrders_({ statuses: ['営業倉庫'] });
  var targetOrders = orders.filter(function(o) {
    return payload.poIds.indexOf(o.po_id) >= 0;
  });
  
  if (targetOrders.length === 0) {
    throw new Error('送信対象の発注がありません（ステータス: 営業倉庫）');
  }
  
  // メール本文生成
  var subject = '【納品手続完了】befree - ' + today_() + ' (' + targetOrders.length + '件)';
  var htmlBody = buildDeliveryEmailHtml_(targetOrders);
  
  // 送信
  MailApp.sendEmail({
    to: CONFIG.MAIL.SUPROLE,
    subject: subject,
    htmlBody: htmlBody
  });
  
  Logger.log('納品完了メール送信: ' + targetOrders.length + '件');
  
  return { success: true, sentCount: targetOrders.length };
}

/**
 * 依頼メールHTML生成
 */
function buildRequestEmailHtml_(orders) {
  var html = '<html><body style="font-family: sans-serif;">';
  html += '<h2>発注依頼</h2>';
  html += '<p>以下の商品について発注をお願いします。</p>';
  html += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">';
  html += '<tr style="background-color: #f0f0f0;">';
  html += '<th>発注ID</th><th>ASIN</th><th>商品コード</th><th>商品名</th>';
  html += '<th>数量（個）</th><th>単価</th><th>合計額</th><th>発注日</th>';
  html += '</tr>';
  
  var totalQuantity = 0;
  var totalAmount = 0;
  
  orders.forEach(function(o) {
    html += '<tr>';
    html += '<td>' + o.po_id + '</td>';
    html += '<td>' + o.asin + '</td>';
    html += '<td>' + o.productCode + '</td>';
    html += '<td>' + o.productName + '</td>';
    html += '<td style="text-align: right;">' + o.quantity + '</td>';
    html += '<td style="text-align: right;">¥' + o.unitPrice.toLocaleString() + '</td>';
    html += '<td style="text-align: right;">¥' + o.subtotal.toLocaleString() + '</td>';
    html += '<td>' + o.orderDate + '</td>';
    html += '</tr>';
    
    totalQuantity += o.quantity;
    totalAmount += o.subtotal;
  });
  
  html += '</table>';
  html += '<p><strong>発注件数: ' + orders.length + '件</strong></p>';
  html += '<p><strong>合計発注数: ' + totalQuantity + '個</strong></p>';
  html += '<p><strong>合計発注額: ¥' + totalAmount.toLocaleString() + '（税抜）</strong></p>';
  html += '</body></html>';
  
  return html;
}

/**
 * 納品完了メールHTML生成
 */
function buildDeliveryEmailHtml_(orders) {
  var html = '<html><body style="font-family: sans-serif;">';
  html += '<h2>納品手続完了通知</h2>';
  html += '<p>以下の商品について納品手続きが完了しました。</p>';
  html += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">';
  html += '<tr style="background-color: #f0f0f0;">';
  html += '<th>発注ID</th><th>ASIN</th><th>商品コード</th><th>商品名</th>';
  html += '<th>数量（個）</th><th>単価</th><th>合計額</th>';
  html += '<th>消費税率</th><th>伝票No.</th><th>到着予定日</th>';
  html += '</tr>';
  
  var totalQuantity = 0;
  var totalSubtotal = 0;
  var totalTax = 0;
  
  orders.forEach(function(o) {
    var tax = o.subtotal * o.taxRate;
    var taxRatePct = (o.taxRate * 100).toFixed(0) + '%';
    
    html += '<tr>';
    html += '<td>' + o.po_id + '</td>';
    html += '<td>' + o.asin + '</td>';
    html += '<td>' + o.productCode + '</td>';
    html += '<td>' + o.productName + '</td>';
    html += '<td style="text-align: right;">' + o.quantity + '</td>';
    html += '<td style="text-align: right;">¥' + o.unitPrice.toLocaleString() + '</td>';
    html += '<td style="text-align: right;">¥' + o.subtotal.toLocaleString() + '</td>';
    html += '<td style="text-align: center;">' + taxRatePct + '</td>';
    html += '<td>' + o.invoiceNo + '</td>';
    html += '<td>' + o.arrivalDate + '</td>';
    html += '</tr>';
    
    totalQuantity += o.quantity;
    totalSubtotal += o.subtotal;
    totalTax += tax;
  });
  
  var totalWithTax = totalSubtotal + totalTax;
  
  html += '</table>';
  html += '<p><strong>発注件数: ' + orders.length + '件</strong></p>';
  html += '<p><strong>合計数量: ' + totalQuantity + '個</strong></p>';
  html += '<p><strong>税抜合計: ¥' + totalSubtotal.toLocaleString() + '</strong></p>';
  html += '<p><strong>消費税: ¥' + Math.round(totalTax).toLocaleString() + '</strong></p>';
  html += '<p><strong>税込合計: ¥' + Math.round(totalWithTax).toLocaleString() + '</strong></p>';
  html += '</body></html>';
  
  return html;
}

/**
 * POSTハンドラ（依頼メール）
 */
function handleSendRequestEmail_(payload) {
  return json_(sendRequestEmail_(payload));
}

/**
 * POSTハンドラ（納品完了メール）
 */
function handleSendDeliveryEmail_(payload) {
  return json_(sendDeliveryEmail_(payload));
}
```

### テスト内容
1. テスト用の発注データを作成
2. 依頼メール送信APIを実行し、メールが届くことを確認
3. 納品完了メール送信APIを実行し、メールが届くことを確認

### 完了条件
✅ メールが正しい宛先に送信される  
✅ HTML形式の本文が正しく表示される  
✅ 集計値（件数・数量・金額）が正確

---

## Phase 4: GASマスタ検索API拡張 🟡 重要

**目的**: 商品検索・マスタ結合ビュー取得機能を実装

### 作業内容

#### 4.1 商品検索API
**ファイル**: `apps/gas/src/handlers/products.js`（新規）

```javascript
/**
 * 商品検索（発注作成用）
 * @param {Object} filters { asin?, productCode?, namePart?, sku? }
 * @return {Array<Object>}
 */
function searchProducts_(filters) {
  filters = filters || {};
  
  // 商品マスタ取得
  var productSheet = openProductMasterSheet_();
  var products = readSheetAsObjects_(productSheet);
  
  // 仕入れマスタ取得
  var purchaseSheet = openPurchaseMasterSheet_();
  var purchases = readSheetAsObjects_(purchaseSheet);
  var purchaseMap = {};
  purchases.forEach(function(p) {
    purchaseMap[String(p.ASIN)] = p;
  });
  
  // フィルタ適用
  var results = products.filter(function(p) {
    // 表示/非表示チェック
    if (p['表示/非表示'] !== true && p['表示/非表示'] !== 'TRUE') return false;
    
    if (filters.sku && String(p.SKU) !== filters.sku) return false;
    if (filters.asin && String(p.ASIN) !== filters.asin) return false;
    if (filters.productCode && String(p['商品コード']) !== filters.productCode) return false;
    if (filters.namePart) {
      var keyword = String(filters.namePart).toLowerCase();
      var name = String(p['商品名'] || '').toLowerCase();
      if (name.indexOf(keyword) === -1) return false;
    }
    
    return true;
  });
  
  // 仕入れマスタと結合
  results = results.map(function(p) {
    var asin = String(p.ASIN);
    var purchase = purchaseMap[asin] || {};
    
    return {
      sku: String(p.SKU || ''),
      asin: asin,
      productCode: String(p['商品コード'] || ''),
      name: String(p['商品名'] || ''),
      brand: String(p['ブランド'] || ''),
      setSize: num_(purchase['セット個数']) || 1,
      minLot: num_(purchase['最小ロット']) || 1,
      purchasePrice: num_(purchase['仕入れ値（税抜/セット）']) || 0,
      hazard: purchase['危険物'] === true || purchase['危険物'] === 'TRUE',
      hasExpiry: purchase['消費期限要'] === true || purchase['消費期限要'] === 'TRUE'
    };
  });
  
  return results;
}

/**
 * GETハンドラ（商品検索）
 */
function handleSearchProducts_(e) {
  var p = e.parameter || {};
  var filters = {
    sku: p.sku,
    asin: p.asin,
    productCode: p.productCode,
    namePart: p.q || p.namePart
  };
  
  var results = searchProducts_(filters);
  
  return json_({ items: results });
}
```

#### 4.2 マスタ結合ビュー
**ファイル**: `apps/gas/src/handlers/products.js`

```javascript
/**
 * マスタ結合ビュー取得
 */
function getMasterView_(filters) {
  return searchProducts_(filters); // 同じロジックを使用
}

/**
 * GETハンドラ（マスタ結合ビュー）
 */
function handleGetMasterView_(e) {
  return handleSearchProducts_(e);
}
```

#### 4.3 Code.jsへのルーティング追加
**ファイル**: `apps/gas/src/Code.js`

```javascript
function doGet(e) {
  try {
    guard_(e);
    var p = (e && e.parameter && e.parameter.path) ? String(e.parameter.path).toLowerCase() : 'dashboard';
    
    // 既存のルーティング
    if (p === 'dashboard') return handleDashboard_(e);
    if (p === 'products')  return handleProducts_(e);
    if (p === 'product')   return handleProductBySku_(e);
    if (p === 'alerts')    return handleAlerts_(e);
    
    // 発注管理用のルーティング追加
    if (p === 'orders') return handleGetOrders_(e);
    if (p === 'searchproducts') return handleSearchProducts_(e);
    if (p === 'masterview') return handleGetMasterView_(e);
    
    return json_({ error: 'not found' }, 404);
  } catch (err) {
    return json_({ error: String(err) }, 500);
  }
}
```

### テスト内容
1. 商品検索APIで部分一致検索が動作することを確認
2. 仕入れマスタとの結合データが正しく返却されることを確認

### 完了条件
✅ 商品検索で複数条件のフィルタが機能する  
✅ 仕入れマスタの情報が正しく結合される

---

## Phase 5: Next.jsサーバルート実装 🟡 重要

**目的**: GAS APIをプロキシするNext.jsサーバルートを実装

### 作業内容

#### 5.1 発注一覧・詳細取得
**ファイル**: `apps/web/app/api/gas/orders/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function GET(req: NextRequest) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const searchParams = req.nextUrl.searchParams;
  
  const url = new URL(base);
  url.searchParams.set('path', 'orders');
  url.searchParams.set('key', key);
  
  // フィルタパラメータを転送
  ['po_id', 'statuses', 'sku', 'asin', 'seller', 'productName', 'fromDate', 'toDate'].forEach(param => {
    const value = searchParams.get(param);
    if (value) url.searchParams.set(param, value);
  });

  try {
    const res = await fetch(url.toString(), { cache: 'no-store' });
    const data = await res.json();
    
    if (data._status && data._status >= 400) {
      return NextResponse.json(data, { status: data._status });
    }
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to fetch orders' },
      { status: 500 }
    );
  }
}
```

#### 5.2 発注作成
**ファイル**: `apps/web/app/api/gas/orders/route.ts`

```typescript
// 同じファイルに追加
export async function POST(req: NextRequest) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const url = new URL(base);
  url.searchParams.set('action', 'createOrder');
  url.searchParams.set('key', key);

  try {
    const payload = await req.json();
    
    const res = await fetch(url.toString(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
    
    const data = await res.json();
    
    if (data._status && data._status >= 400) {
      return NextResponse.json(data, { status: data._status });
    }
    
    // 成功時はキャッシュをクリア（オプション）
    // revalidatePath('/orders');
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to create order' },
      { status: 500 }
    );
  }
}
```

#### 5.3 発注詳細取得
**ファイル**: `apps/web/app/api/gas/orders/[poId]/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function GET(
  req: NextRequest,
  { params }: { params: { poId: string } }
) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const url = new URL(base);
  url.searchParams.set('path', 'orders');
  url.searchParams.set('key', key);
  url.searchParams.set('po_id', params.poId);

  try {
    const res = await fetch(url.toString(), { cache: 'no-store' });
    const data = await res.json();
    
    if (data.items && data.items.length > 0) {
      return NextResponse.json(data.items[0]);
    }
    
    return NextResponse.json({ error: 'Order not found' }, { status: 404 });
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to fetch order' },
      { status: 500 }
    );
  }
}
```

#### 5.4 発注更新
**ファイル**: `apps/web/app/api/gas/orders/[poId]/route.ts`

```typescript
// 同じファイルに追加
export async function PUT(
  req: NextRequest,
  { params }: { params: { poId: string } }
) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const url = new URL(base);
  url.searchParams.set('action', 'updateOrder');
  url.searchParams.set('key', key);

  try {
    const payload = await req.json();
    payload.po_id = params.poId;
    
    const res = await fetch(url.toString(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
    
    const data = await res.json();
    
    if (data._status && data._status >= 400) {
      return NextResponse.json(data, { status: data._status });
    }
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to update order' },
      { status: 500 }
    );
  }
}
```

#### 5.5 ステータス変更
**ファイル**: `apps/web/app/api/gas/orders/[poId]/status/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function PUT(
  req: NextRequest,
  { params }: { params: { poId: string } }
) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const url = new URL(base);
  url.searchParams.set('action', 'changeStatus');
  url.searchParams.set('key', key);

  try {
    const payload = await req.json();
    payload.po_id = params.poId;
    
    const res = await fetch(url.toString(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
    
    const data = await res.json();
    
    if (data._status && data._status >= 400) {
      return NextResponse.json(data, { status: data._status });
    }
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to change status' },
      { status: 500 }
    );
  }
}
```

#### 5.6 メール送信
**ファイル**: `apps/web/app/api/gas/orders/mail/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function POST(req: NextRequest) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  try {
    const { type, poIds } = await req.json();
    
    if (!['request', 'delivery'].includes(type)) {
      return NextResponse.json(
        { error: 'Invalid mail type' },
        { status: 400 }
      );
    }
    
    const action = type === 'request' ? 'sendRequestEmail' : 'sendDeliveryEmail';
    
    const url = new URL(base);
    url.searchParams.set('action', action);
    url.searchParams.set('key', key);
    
    const res = await fetch(url.toString(), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ poIds }),
    });
    
    const data = await res.json();
    
    if (data._status && data._status >= 400) {
      return NextResponse.json(data, { status: data._status });
    }
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to send email' },
      { status: 500 }
    );
  }
}
```

#### 5.7 商品検索
**ファイル**: `apps/web/app/api/gas/products/search/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function GET(req: NextRequest) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const searchParams = req.nextUrl.searchParams;
  
  const url = new URL(base);
  url.searchParams.set('path', 'searchproducts');
  url.searchParams.set('key', key);
  
  // 検索パラメータを転送
  ['sku', 'asin', 'productCode', 'q'].forEach(param => {
    const value = searchParams.get(param);
    if (value) url.searchParams.set(param, value);
  });

  try {
    const res = await fetch(url.toString(), { cache: 'no-store' });
    const data = await res.json();
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to search products' },
      { status: 500 }
    );
  }
}
```

#### 5.8 マスタビュー
**ファイル**: `apps/web/app/api/gas/masters/route.ts`（新規）

```typescript
import { NextRequest, NextResponse } from 'next/server';

export async function GET(req: NextRequest) {
  const base = process.env.GAS_API_BASE!;
  const key = process.env.GAS_API_KEY!;

  const searchParams = req.nextUrl.searchParams;
  
  const url = new URL(base);
  url.searchParams.set('path', 'masterview');
  url.searchParams.set('key', key);
  
  // 検索パラメータを転送
  ['sku', 'asin', 'productCode', 'q'].forEach(param => {
    const value = searchParams.get(param);
    if (value) url.searchParams.set(param, value);
  });

  try {
    const res = await fetch(url.toString(), { cache: 'no-store' });
    const data = await res.json();
    
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to fetch masters' },
      { status: 500 }
    );
  }
}
```

### テスト内容
1. 各APIルートが正常に動作することを確認
2. エラーハンドリングが適切に行われることを確認

### 完了条件
✅ 全APIルートが実装され、GASへのプロキシが動作する  
✅ エラー時のレスポンスが適切

---

## Phase 6: 型定義とスキーマ整備 🟡 重要

**目的**: TypeScript型定義を追加し、型安全性を確保

### 作業内容

#### 6.1 型定義追加
**ファイル**: `apps/web/lib/types.ts`

```typescript
// 既存のファイルに追加

// Order types
export type OrderStatus =
  | '依頼中'
  | '営業倉庫'
  | 'FBA'
  | '返品';

export type Order = {
  po_id: string;
  sku: string;
  asin: string;
  productCode: string;
  productName: string;
  orderDate: string; // YYYY-MM-DD
  seller: string;
  quantity: number;
  setCount: number;
  setSize: number;
  unitPrice: number;
  subtotal: number;
  taxRate: number;
  invoiceNo: string;
  arrivalDate: string | null;
  status: OrderStatus;
  remarks: string;
  createdBy: string;
  createdAt: string;
  lastUpdatedBy: string;
  lastUpdatedAt: string;
};

export type OrdersResponse = { items: Order[]; total: number } | ApiError;

export type CreateOrderInput = {
  sku: string;
  setCount: number;
  taxRate?: number;
  seller?: string;
  orderDate?: string;
  remarks?: string;
  createdBy: string;
};

export type UpdateOrderInput = {
  taxRate?: number;
  invoiceNo?: string;
  arrivalDate?: string;
  remarks?: string;
  updatedBy: string;
};

export type ChangeStatusInput = {
  newStatus: OrderStatus;
  updatedBy: string;
};

export type SendMailInput = {
  type: 'request' | 'delivery';
  poIds: string[];
};

// Product search types
export type ProductSearchResult = {
  sku: string;
  asin: string;
  productCode: string;
  name: string;
  brand: string;
  setSize: number;
  minLot: number;
  purchasePrice: number;
  hazard: boolean;
  hasExpiry: boolean;
};

export type ProductSearchResponse = { items: ProductSearchResult[] } | ApiError;

// Master view types
export type MasterItem = ProductSearchResult; // 同じ構造
export type MasterViewResponse = { items: MasterItem[] } | ApiError;
```

### 完了条件
✅ 型定義が追加され、型エラーがない

---

## Phase 7-9: UI実装

（詳細は長くなるため、別途指示で展開します）

### Phase 7: 発注一覧ページ（/orders）
- フィルタ・ソート・ページネーション
- 行選択・一括操作
- ステータス変更
- 編集モーダル

### Phase 8: 新規発注作成ページ（/orders/create）
- 商品検索
- セット数入力・計算
- 確認・作成

### Phase 9: マスタ閲覧ページ（/masters）
- 商品・仕入れマスタの結合ビュー
- 検索・フィルタ

---

## Phase 10: 統合・テスト・デプロイ 🟢 仕上げ

### 作業内容
1. E2Eテスト実施
2. 権限テスト（allowlist）
3. パフォーマンステスト
4. ドキュメント整備
5. デプロイ（dev → prod）

---

## 📊 進捗管理

各フェーズ完了時にチェック：

```
□ Phase 1: GAS基盤整備
□ Phase 2: GAS発注CRUD API
□ Phase 3: GASメール送信API
□ Phase 4: GASマスタ検索API拡張
□ Phase 5: Next.jsサーバルート
□ Phase 6: 型定義とスキーマ
□ Phase 7: UI - 発注一覧
□ Phase 8: UI - 新規発注作成
□ Phase 9: UI - マスタ閲覧
□ Phase 10: 統合・テスト・デプロイ
```

---

## 🎯 次のアクション

**推奨**: Phase 1から順次実装を開始してください。

各フェーズ完了後、動作確認とテストを行い、次のフェーズに進みます。
問題が発生した場合は、その都度調整します。

実装を開始する準備はできていますか？どのフェーズから着手しますか？

